...
export const useSelect = ({
  ...
  React.useEffect(() => {
    ...
    const searchOption = filteredOptions.find(
      (option) => searchSelectedOption.id === option.id
    );
    if (!searchOption)
      return setSearchSelectedOption({ index: 0, id: filteredOptions[0].id });

    setSearchSelectedOption({
      id: searchOption.id,
      index: filteredOptions.findIndex(
  ...
  React.useLayoutEffect(() => {
    if (ulRef.current && searchSelectedOption) {
      ...
      ulRef.current.scrollTop =
        optionHeight * searchSelectedOption.index - optionHeight;
    }
